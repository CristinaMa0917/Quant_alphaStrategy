...
def prepare(filename):
    signal_df = pd.read_csv(filename, dtype={"ticker": np.str, "tradeDate": np.str},index_col=0)
...
df = prepare('alpha_ml/stacking_xgb0524.csv')
results = backtesting(df)
show_diff(results)
t = show_table(results)
t


# In[ ]:

df = prepare('alpha_ml/stacking_xgb72_lr6.csv')
results = backtesting(df)
show_diff(results)
t = show_table(results)
t
...
df = prepare('alpha_ml/stacking_xgb72_rf24.csv')
results = backtesting(df)
show_diff(results)
t = show_table(results)
t
